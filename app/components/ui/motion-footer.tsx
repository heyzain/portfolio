...
export interface CinematicFooterProps {
  /** Huge watermark word rendered behind the footer. */
  giantText?: string;
  /** Center headline / closing CTA. */
  heading?: string;
  /** Prominent pill CTAs, each with an optional leading icon. */
  primaryLinks?: CinematicFooterLink[];
  /** Smaller secondary pill links. */
  secondaryLinks?: CinematicFooterLink[];
  /** Phrases scrolled in the diagonal marquee. */
  marqueeItems?: string[];
  /** Name shown in the "crafted with ❤ by …" badge. */
  author?: string;
  /** Full copyright line. */
  copyright?: string;
}
...
const DEFAULT_PRIMARY: CinematicFooterLink[] = [
  { label: "Download iOS", href: "#", icon: AppleIcon },
  { label: "Download Android", href: "#", icon: AndroidIcon },
];

const DEFAULT_SECONDARY: CinematicFooterLink[] = [
...
export function CinematicFooter({
  ...
  author = "Volvox",
  copyright = "© 2026 Volvox. All rights reserved.",
              ...
              {primaryLinks.length > 0 && (
                <div className="flex flex-wrap justify-center gap-4 w-full">
                  {primaryLinks.map((link) => (
                    ...
                    >
                      {link.icon && (
                        <span className="flex h-6 w-6 items-center justify-center text-muted-foreground group-hover:text-foreground transition-colors">
                          {link.icon}
                        </span>
                      )}
                      {link.label}
