...
export function ContactDispatch() {
  ...
  const inputClass = "mt-2 w-full rounded-[16px] border border-ink/12 bg-white/86 px-5 py-4 font-mono text-sm text-ink shadow-inner outline-none placeholder:text-ink/32 transition focus:border-olive/55 focus:bg-white focus:ring-4 focus:ring-olive/10 disabled:cursor-not-allowed disabled:opacity-65";
...
function Terminal({ phase, lines, reference, name, email, message }: { phase: Phase; lines: string[]; reference: string; name: string; email: string; message: string }) {
  const sending = phase === "sending";
  return (
    <motion.aside animate={phase === "completed" ? { boxShadow: ["0 22px 56px -38px rgba(17,17,17,.8)", "0 0 34px rgba(132,204,22,.22)", "0 22px 56px -38px rgba(17,17,17,.8)"] } : {}} transition={{ duration: 1.1 }} className={cn("relative flex min-h-[340px] flex-col justify-between overflow-hidden rounded-[22px] border bg-ink p-5 font-mono text-[12px] leading-relaxed shadow-[0_22px_56px_-38px_var(--ink)] md:p-6 lg:col-span-2", phase === "completed" ? "border-olive/40" : "border-ink/85 text-[#f7f1dd]")} aria-live="polite">
      <div>
        <div className="mb-5 flex gap-2" aria-hidden>
          {["#ef4444", "#fbbf24", "#22c55e"].map((color, index) => <motion.span key={color} className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: color }} animate={sending ? { opacity: [0.35, 1, 0.35], scale: [1, 1.12, 1] } : { opacity: phase === "idle" ? (index === 0 ? 1 : 0.25) : 0.8 }} transition={{ duration: 0.8, repeat: sending ? Infinity : 0, delay: index * 0.14 }} />)}
        </div>
        {phase === "idle" ? (
          <div><p><span className="text-[#f59e0b]">to:</span> <span className="text-[#e7e0cc]">{profile.email}</span></p><p className="mt-4"><span className="text-[#84cc16]">from:</span> {name || <span className="text-[#d8d0bd]/70">awaiting input</span>}</p><p><span className="text-[#38bdf8]">reply:</span> {email || <span className="text-[#d8d0bd]/70">awaiting input</span>}</p><p className="mt-4 whitespace-pre-wrap break-all text-[#e7e0cc]"><span className="text-[#fb7185]">body:</span> {message || <span className="text-[#f7f1dd]/40">say something useful</span>}<span className="animate-caret ml-0.5 inline-block h-4 w-2 translate-y-0.5 bg-[#facc15]" /></p></div>
        ) : (
          <div className="flex flex-col gap-2 text-[11px]">{lines.map((line, index) => <p key={`${index}-${line}`} className={line.includes("successfully") ? "font-semibold text-[#84cc16]" : failures.includes(line) ? "text-[#fb7185]" : "text-[#a8a29e]"}>{line}{index === lines.length - 1 && sending && <span className="animate-caret ml-1 inline-block h-3.5 w-1.5 translate-y-0.5 bg-[#facc15]" />}</p>)}</div>
        )}
      </div>
      <div className={cn("mt-6 border-t border-white/10 pt-4 text-[10px] leading-relaxed", phase === "error" ? "text-[#fb7185]/70" : phase === "completed" ? "text-[#84cc16]/70" : "text-[#d8d0bd]/50")}>{phase === "idle" ? "> Ready for input..." : sending ? "> Processing secure dispatch..." : phase === "completed" ? `> Delivery confirmed · ${reference}` : "> Dispatch paused. Retry available."}</div>
      {phase === "completed" && <motion.div initial={{ opacity: 0, x: -4, y: 4 }} animate={{ opacity: [0, 0.75, 0], x: 26, y: -26 }} transition={{ duration: 0.9, ease: "easeOut" }} className="pointer-events-none absolute right-6 top-8 text-[#84cc16]" aria-hidden><SendIcon /></motion.div>}
    </motion.aside>
  );
}
